...
pre { /* Code block - determines code spacing between lines */
 ...
 Q1: MSEs look very different than what is expected. There might be some problems in your model fits: no need for extra x, for example, in lm(y ~ x + poly(x, 2, raw = T), data = test). x and x^2 are already included with poly(). Q2: LRT calculations are not right. Q5b: What would you tell about the relationship between z and y and the quadratic term x+x^2?
